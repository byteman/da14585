...
void spi_flash_size_init(void)
{
    ...
    switch(man_dev_id)
    {
        ...
        case W25X20CL_MANF_DEV_ID:
            ...
            break; 
        case MX25R2035F_MANF_DEV_ID:
            flash_size = MX25R2035F_SIZE;
            flash_page = MX25R2035F_PAGE;
            break;
        default:
...
****************************************************************************************
*/
inline void getByte_SPI(uint8_t *x)
{
    const uint16_t DUMMY = 0xffff;
    SetWord16(SPI_RX_TX_REG0, (uint16_t) DUMMY);
    while (GetBits16(SPI_CTRL_REG, SPI_INT_BIT==0));
    SetWord16(SPI_CLEAR_INT_REG, 0x01);
    *(uint8_t *)x=(uint8_t)GetWord16(SPI_RX_TX_REG0);   
} 
...
void SpiFlashRead(unsigned long destination_buffer,unsigned long source_addr,unsigned long len)
{
    ...
    SetWord16(SPI_RX_TX_REG0, (uint16_t) 0);
    ...
    for (i=0;i<len;i++)
    {
        ...
        __ASM volatile ("nop");
        __ASM volatile ("nop");
        *(uint8_t *)(uint32_t)(destination_buffer+i)=(uint8_t)GetWord16(SPI_RX_TX_REG0);
    }
    spi_cs_high();
} 
