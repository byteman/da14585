...
int starttest_tx_param_len_5_handler(int argc, char **argv)
{
	...
	if ( !( evt->event == 0x0F
		&& evt->length == 4
		&& evt->parameters[0] == 0
	        && (evt->parameters[2] == (HCI_TX_TEST_CMD_OPCODE & 0x00FF))
	        && (evt->parameters[3] == HCI_TX_TEST_CMD_OPCODE>>8))
	)
	{
		return_status = SC_UNEXPECTED_EVENT; // unexpected event
		goto exit_command_handler;
	...
	if ( !( evt2->event == 0x0E
		&& evt2->length == 3
                && (evt2->parameters[1] == (HCI_TX_TEST_CMD_OPCODE & 0x00FF))
                && (evt2->parameters[2] == HCI_TX_TEST_CMD_OPCODE>>8))
	)
	{
		return_status = SC_UNEXPECTED_EVENT; // unexpected event
		goto exit_command_handler;
