...
__INLINE struct gapc_security_cmd* app_gapc_security_request_msg_create(uint8_t conidx, enum gap_auth auth)
{
    struct gapc_security_cmd * cmd = KE_MSG_ALLOC(GAPC_SECURITY_CMD,
                KE_BUILD_ID(TASK_GAPC, conidx), TASK_APP, gapc_security_cmd);
 ...
 *                              enabled.
 * @param[in] bd_addr           Provided own static private random address if addr_type is
 *                              GAPM_CFG_ADDR_PRIVATE.
 * @param[in] irk               Pointer to an array that contains the device Identity
 *                              Resolving Key (IRK).
 * @param[in] priv1_2           Privacy 1.2 feature.
