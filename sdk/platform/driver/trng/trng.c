...
static uint8_t rfadc_data[NUM_POINTS*2] __attribute__((section("trng_buffer_area_zi"), zero_init));
 ...
 ****************************************************************************************
 */
static void trng_get_raw_data(uint32_t address, uint32_t length)
{
...
void trng_acquire(uint8_t *trng_bits_ptr)
{
    ...
    {
        trng_get_raw_data((uint32_t)&rfadc_data[0], NUM_POINTS/2-1); // acquires the raw RFADC IQ samples
