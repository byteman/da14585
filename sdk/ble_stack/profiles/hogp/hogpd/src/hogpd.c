...
const struct attm_desc hids_att_db[HOGPD_IDX_NB] =
{
    ...
    [HOGPD_IDX_SVC]                             = {ATT_DECL_PRIMARY_SERVICE, PERM(RD, ENABLE), 0},

    // HID Service Declaration
    [HOGPD_IDX_INCL_SVC]                        = {ATT_DECL_INCLUDE, PERM(RD, ENABLE), 0},

    // HID Information Characteristic Declaration
    [HOGPD_IDX_HID_INFO_CHAR]                   = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE),0},
    // HID Information Characteristic Value
    [HOGPD_IDX_HID_INFO_VAL]                    = {ATT_CHAR_HID_INFO, PERM(RD, ENABLE), sizeof(struct hids_hid_info)},

    // HID Control Point Characteristic Declaration
    [HOGPD_IDX_HID_CTNL_PT_CHAR]                = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0},
    // HID Control Point Characteristic Value
    [HOGPD_IDX_HID_CTNL_PT_VAL]                 = {ATT_CHAR_HID_CTNL_PT, PERM(WR, ENABLE) | PERM(WRITE_COMMAND, ENABLE), PERM(RI, ENABLE) |  sizeof(uint8_t)},
    ...
    [HOGPD_IDX_REPORT_MAP_EXT_REP_REF]          = {ATT_DESC_EXT_REPORT_REF, PERM(RD, ENABLE), sizeof(uint16_t)},
    ...
    [HOGPD_IDX_PROTO_MODE_VAL]                  = {ATT_CHAR_PROTOCOL_MODE, (PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(WRITE_COMMAND, ENABLE)), PERM(RI, ENABLE) | sizeof(uint8_t)},

    // Boot Keyboard Input Report Characteristic Declaration
    [HOGPD_IDX_BOOT_KB_IN_REPORT_CHAR]          = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0},
    ...
    [HOGPD_IDX_BOOT_KB_IN_REPORT_NTF_CFG]       = {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE), 0},

    // Boot Keyboard Output Report Characteristic Declaration
    [HOGPD_IDX_BOOT_KB_OUT_REPORT_CHAR]         = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0},
    // Boot Keyboard Output Report Characteristic Value
    [HOGPD_IDX_BOOT_KB_OUT_REPORT_VAL]          = {ATT_CHAR_BOOT_KB_OUT_REPORT, (PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE)), PERM(RI, ENABLE) | HOGPD_BOOT_REPORT_MAX_LEN},

    // Boot Mouse Input Report Characteristic Declaration
    [HOGPD_IDX_BOOT_MOUSE_IN_REPORT_CHAR]       = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0},
    ...
    [HOGPD_IDX_BOOT_MOUSE_IN_REPORT_NTF_CFG]    = {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE), 0},

    // Report Characteristic Declaration
    [HOGPD_IDX_REPORT_CHAR]                     = {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0},
    ...
    [HOGPD_IDX_REPORT_REP_REF]                  = {ATT_DESC_REPORT_REF, PERM(RD, ENABLE), sizeof(struct hids_report_ref)},
    // Report Characteristic - Client Characteristic Configuration Descriptor
    [HOGPD_IDX_REPORT_NTF_CFG]                  = {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE), 0},
};
...
static uint8_t hogpd_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct hogpd_db_cfg* params)
{
    ...
    for (svc_idx = 0; ((svc_idx < params->hids_nb) && (status == GAP_ERR_NO_ERROR)); svc_idx++)
    {
        ...
        if ((params->cfg[svc_idx].svc_features & HOGPD_CFG_KEYBOARD) == HOGPD_CFG_KEYBOARD)
        {
            ...
            if ((params->cfg[svc_idx].svc_features & HOGPD_CFG_BOOT_KB_WR) == HOGPD_CFG_BOOT_KB_WR)
            {
                ...
                hids_db[svc_idx][HOGPD_IDX_BOOT_KB_IN_REPORT_VAL].perm |= (PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE));
            }
        }
        ...
        if ((params->cfg[svc_idx].svc_features & HOGPD_CFG_MOUSE) == HOGPD_CFG_MOUSE)
        {
            ...
            {
                // Adds write permissions on report
                hids_db[svc_idx][HOGPD_IDX_BOOT_MOUSE_IN_REPORT_VAL].perm |= (PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE));
            }
        }
        ...
        for (report_idx = 0; report_idx < params->cfg[svc_idx].report_nb; report_idx++)
        {
            ...
            switch (params->cfg[svc_idx].report_char_cfg[report_idx] & HOGPD_CFG_REPORT_FEAT)
            {
                ...
                {
                    perm = PERM(RD, ENABLE) | PERM(WR, ENABLE)| PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE);
                } break;
    ...
    for (svc_idx = 0; ((svc_idx < params->hids_nb) && (status == GAP_ERR_NO_ERROR)); svc_idx++)
    {
        ...
        if(status == GAP_ERR_NO_ERROR)
        {
            ...
            status = attmdb_att_set_value(handle, sizeof(struct hids_hid_info), 0, (uint8_t *)&params->cfg[svc_idx].hid_info);
        }
                ...
                && ((params->cfg[svc_idx].svc_features & HOGPD_CFG_MAP_EXT_REF) == HOGPD_CFG_MAP_EXT_REF))
        {
            ...
            ASSERT_ERR(handle != ATT_INVALID_HDL);
            status = attmdb_att_set_value(handle, sizeof(uint16_t), 0, (uint8_t *)&params->cfg[svc_idx].ext_ref.rep_ref_uuid);
        }
                              ...
                              && (report_idx < params->cfg[svc_idx].report_nb) ; report_idx++)
        {
            ...
            status = attmdb_att_set_value(handle, sizeof(struct hids_report_ref), 0, (uint8_t *)&report_ref);
        }
