...
//        if (co_list_size(&llm_le_env.adv_list) == BLE_DUPLICATE_FILTER_MAX)	//BUG make size configurable
        if (co_list_size(&llm_le_env.adv_list) == ble_duplicate_filter_max)	//BUG make size configurable
        {
            // Consider the device as found
//BUG            found = true;
            found = ble_duplicate_filter_found;	//by default this is true so devices exceeding max are found and will not be reported
        }
        else
        {
